...
namespace GraphTool
{
    ...
    private:
        ...
        static void EdgeConsumer(void* arg)
        {
            ...
            while (true)
            {
                ...
                WriteEdgesToFile(writeSpec->graphfile, writeSpec->bufs[currentIndex], writeSpec->counts[currentIndex], writeSpec->groupedByDestination);
    ...
    private:
        ...
        /// @param [in] count Number of edges contained in the buffer.
        /// @param [in] groupedByDestination Indicates that graph edges should be grouped by destination instead of by source.
        /// @return `true` if all edges were successfully written, `false` otherwise.
        virtual bool WriteEdgesToFile(FILE* graphfile, SEdgeBufferData<TEdgeData>* buf, size_t count, bool groupedByDestination) = 0;
